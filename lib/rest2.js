...
class RESTv2 {
  ...
  /**
   * Get recurring algo orders status
   *
   * @param {object} args
   * @param {number} [args.limit]
   * @param {'active' | 'canceled'} [args.status]
   * @param {Function} [cb] - callback
   * @returns {Promise<AlgoOrder[]>}
   */
  getRecurringAlgoOrdersStatus (args, cb) {
    return this._makeAuthRequest('/auth/r/ext/recurring-ao/status', args, cb)
  }
