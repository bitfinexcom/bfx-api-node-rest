...
class RESTv2 {
  ...
  keepFunding (params, cb) {
    return this._makeAuthRequest('/auth/w/funding/keep', params, cb, FundKeeper)
  }
   ...
   * Cancel orders.
   *
   * @param {number[]} id   - list of order ids to cancel
   * @param {Function} [cb] - callback
   * @returns {Promise} p
   */
  cancelOrders (ids = [], cb) {
   ...
   *
   * @param {object} params - Multiple order-related operations parameters
   * @param {Array.<Array.<string,object>>} params.ops - List of operations to perform
   * @param {Array.<string,Order>} [params.ops[0]] - Order Submit ['on', new Order({...})]
   * @param {Array.<string,{id:number}>} [params.ops[1]] - Order cancellation ['oc', { id: 1 }]
   * @param {Array.<string,{id:Array.<number>}>} [params.ops[2]] - Multi Order Cancellation ["oc_multi", {id: [1, 2, ...]}]
   * @param {Array.<string,Order>} [params.ops[3]] - Order Update ["ou", {id: 1, price: '15', amount: '0.001'}]
   * @param {Function} [cb] - callback
   * @returns {Promise} - p
   * @see https://docs.bitfinex.com/reference#rest-auth-order-multi
   */
  orderMultiOp (params = { ops: [] }, cb) {
    params.ops = params.ops.map((op) => {
      if (op[0] === 'on' && op[1]) {
        // generate new packet from instance of Order class to ensure consistent payload signature
        const packet = (op[1] instanceof Order ? op[1] : new Order(op[1])).toNewOrderPacket()
        if (this._affCode) {
          packet.meta.aff_code = packet.meta.aff_code || this._affCode
        }
    ...
    return this._makeAuthRequest('/auth/w/order/multi', params, cb)
      .then(_takeResNotify)
  }
   ...
   * @param {number[]} [params.id]  - { id: [1,2,3] }
   * @param {number[]} [params.gid] - { gid: [1,2,3] }
   * @param {number} [params.all]   - { all: 1 }
   * @param {Array.<Array.<number,string>>} [params.cid] - { cid: [[1, '2020-10-10']] }
   * @param {Function} [cb]       - callback
   * @returns {Promise} p
