...
const HTTP_STATUS_SERVER_ERROR_FAMILY = 500
...
class RESTv2 {
   ...
   * @param {number} [opts.retry] - number of times to retry calling the endpoint in case of an error, default 0
   * @param {number} [opts.retryInterval] - default 1000 ms
   */
  constructor (opts = {
    affCode: null,
    apiKey: '',
    ...
    this._timeout = _isInteger(opts.timeout)
      ? opts.timeout
      : BASE_TIMEOUT
    this._maxRetries = opts.retry
    this._retryInterval = opts.retryInterval
  }
   ...
   * @param {number} _tries - to keep track of retries, starts at zero
   * @returns {Promise} p
   * @private
   */
  _makeAuthRequest (path, payload = {}, cb, transformer, _tries = 0) {
    ...
    return rp({
      ...
      .catch((err) => {
        if (err.statusCode >= HTTP_STATUS_SERVER_ERROR_FAMILY && (_tries + 1) < this._maxRetries) {
          return this._delayRetry()
            .then(() => this._makeAuthRequest(path, payload, cb, transformer, _tries + 1))
        }
        throw err
      })
  }
   ...
   * @private
   */
  _makePublicRequest (path, cb, transformer, _tries = 0) {
    const url = `${this._url}/v2${path}`
    ...
    return rp({
      ...
      .catch((err) => {
        if (err.statusCode >= HTTP_STATUS_SERVER_ERROR_FAMILY && (_tries + 1) < this._maxRetries) {
          return this._delayRetry()
            .then(() => this._makePublicRequest(path, cb, transformer, _tries + 1))
        }
        throw err
      })
  }
